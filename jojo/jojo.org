...
*** [note]
    ...
    - x -
      if we do so much for python
      will it makes jojo so bound to python
      so it is only use-able in python ?

    - k -
      maybe we should do datatype and function call,
      but we should not mimic python's syntax.

    - x -
      how about do a jojo-in-javascript now ?
      or jojo-in-rust ?
      or just fix the jojo-in-c ?
...
*** (create)
    ...
    - example : (create <trunk-obj> .field1 ... .field2 ...)
