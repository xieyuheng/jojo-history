...
*** about active actor in scheduler

    - currently a scheduler does not care about an actor,
      if there are no message send to it.

    - this should not be the case.

*** main interpreter process as a scheduler
...
***** preparing
      ...
      3. distribute messages in in_queue to actors,
         and add those actors to active_set.

         - this will gerrentee that
           later each actor in active_queue
           has some messages to be processed.

      4. put actors in active_set to active_queue
